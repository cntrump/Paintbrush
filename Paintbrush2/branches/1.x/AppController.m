...
- (id)init
{
	...
	if (NSAppKitVersionNumber < 824) {
		...
		NSRunAlertPanel(@"Sorry, this program requires Mac OS X 10.4 or later", @"You are running %@", 
		...
		// Here's a big change: only update to version 2.x if we are running Leopard
		if (NSAppKitVersionNumber < 949) {
			// Sorry, stuck with the 1.x branch!
			[defaultValues setObject:@"http://paintbrush.sourceforge.net/updates1x.xml" forKey:@"SUFeedURL"];
		} else {
			[defaultValues setObject:@"http://paintbrush.sourceforge.net/updates2x.xml" forKey:@"SUFeedURL"];
		}
